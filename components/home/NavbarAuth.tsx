...
/**
 * Client component: shows Login + Register when logged out,
 * or an avatar dropdown with Profile + Sign out when logged in.
 */
export function NavbarAuth() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {/* Avatar */}
        <span className="w-8 h-8 flex items-center justify-center text-xs font-black bg-[#E9A319] text-[#1C1917] border-2 border-[#1C1917]/45 group-hover:shadow-[2px_2px_0_rgba(28,25,23,0.18)] transition-all sketchy-border">
          {initials}
        </span>
      ...
      {open && (
        ...
        >
          ...
          <Link
            ...
            role="menuitem"
            onClick={() => setOpen(false)}
            className="flex items-center gap-2 px-4 py-2.5 text-sm font-black text-[#1C1917] bg-[#E9A319]/15 hover:bg-[#E9A319]/30 border-y border-[#E9A319]/30 transition-colors focus-visible:outline-none focus-visible:bg-[#E9A319]/30"
          >
            <span aria-hidden className="text-[#E9A319]">⚡</span>
            Upgrade to Pro
          </Link>
          <form action={logout}>
            <button
